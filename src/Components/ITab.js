...
class ITab extends Component {
  ...
  filter() {
    const { transactions, target, type } = this.props;
    if(transactions.length === 0)
      return {};

    const filtered = transactions.filter(data => {
      const date = new Date(data.date);
      return this.filterAction(date, target, type);
    });
  ...
  render() {
    ...
    const { sorted, filtered } = this.filter();
    if(!sorted)
      return this.empty();
    return (
      <Content>
        {this.overview(filtered)}
        {
          Object.keys(sorted).map((date, index) => {
            const data = sorted[date];
            return (
                ...
                </CardItem>
                {
                  data.map((dat, index) => {
                    ...
                    return (
                      <TouchableNativeFeedback
                        key={index}
                        onPress={() => navigation.navigate("DetailTrans", {
                          id: dat.id
                        })}
                      >
                        <CardItem>
